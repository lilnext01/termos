	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `*Halo @${num.split('@')[0]} selamat datang di grup ${mdata.subject} semoga betah*`
	...
	client.on('message-new', async (mek) => {
		try {
			...
			mess = {
				...
				success: 'Sukses lur',
				error: {
					stick: '[❗] Terjadi kesalahan, mungkin error di sebabkan oleh sistem.',
					Iv: 'Kirim link yang bener'
				},
				only: {
					group: 'Perintah ini hanya bisa di gunakan dalam group!',
					ownerG: 'Cuma owner grup doang yang bisa',
					ownerB: 'Ga owner bot jangan sok keras',
					admin: 'Hanya untuk admin grup',
					Badmin: 'Tolong jadikan bot admin grup'
				}
			}

			const botNumber = client.user.jid
			const ownerNumber = ["6289530863358@s.whatsapp.net"] // replace this with your number
			const adminbotnumber = ["6289530863358@s.whatsapp.net"]
			const frendsowner = ["6285157226383@s.whatsapp.net"]
...
-❥ *${prefix}hidetag*`
				...
				case 'sticker':
					...
					break
			case 'owner':
                 client.sendMessage(from, {displayname: "Galih", vcard: vcard}, MessageType.contact, { quoted: mek})
				...
				case 'hidetag':
					if (!isGroup) return reply(mess.only.group)
					if (!isOwner) return reply('mwehehehe kasian ga bisa')
				...
				case 'block':
				 ...
				 client.chatRead (from)
					...
					client.sendMessage(from, `Sukses memblokir ${body.slice(7)}@c.us`, text)
					break
                    case 'unblock':
					if (!isGroup) return reply(mess.only.group)
					if (!isOwner) return reply(mess.only.ownerB)
                ...
                case 'quotemaker':
					...
					var wm = gh.split("|")[1];
					var bg = gh.split("|")[2];
					const pref = `Pemakaian ${prefix}quotemaker |isi quote|author|theme\n\ncontoh: ${prefix}quotemaker |aku sayang kamu|-aqulzz|random\n\nuntuk theme nya pakai random ya kak..`
				...
				case 'watercolour':
					...
					var gbl3 = gh.split("|")[0];
					var gbl4 = gh.split("|")[1];
					if (args.length < 1) return reply('hadehhh')
					reply(mess.wait)
				...
				case 'thunder':
					if (args.length < 1) return reply('Teksnya mana um')
					thun = body.slice(9)
					if (thun.length > 10) return reply('Teksnya kepanjangan, maksimal 10 karakter')
					reply(mess.wait)
					ane = await fetchJson(`https://zeksapi.herokuapp.com/api/tlight?text=${thun}&apikey=apivinz`, {method: 'get'})
					buffer = await getBuffer(ane.result)
               ...
               client.reply(from, `Waalaikumsalam ${pushname}:)`)
                ...
                case 'donasi':
				case 'donate':
					client.sendMessage(from, 'Donasi? pc owner', text, { quoted: mek })
					break
                case 'tes':
                ...
                case 'report':
                    ...
                    client.sendMessage('6289530863358@s.whatsapp.net', options, text, {quoted: mek})
                    reply('ok sip udah dilaporkan ke owner')
                    break
                case 'request':
                    ...
                    client.sendMessage('6289530863358@s.whatsapp.net', options, text, {quoted: mek})
                    reply('ok sip sudah dikirim ke owner.')
                    break
				case 'meme':
               ...
               case 'infogempa':
                ...
                case 'infogithub':
                   ...
                   break
                case 'infocuaca':
                   anu = await fetchJson(`http://tobz-cuaca.herokuapp.com/?menu=cuaca&wilayah=${body.slice(11)}&apiKey=SLpvUgOcMYwIx0pFeELt`, {method: 'get'})
                   if (anu.error) return reply(anu.error)
				...
				case 'tulis':
					...
					tulis = body.slice(6)
					reply(mess.wait)
					nolis = await fetchJson(`https://arugaz.herokuapp.com/api/nulis?text=${encodeURIComponent(tulis)}`, {method: 'get'})
                    buff = await getBuffer(nolis.result)
					client.sendMessage(from, buff, image, {quoted: mek, caption: 'Jangan malas njir'})
					break
				case 'text3d':
				...
				case 'playstore':
					...
					break
			    case 'cekjodoh':    
					if(args.length < 2) return reply(`Kirim ${prefix}cekjodoh namamu namapasangan, contoh ${prefix}cekjodoh aku dia`)
					akuh = args[0]
					kamuh = args[1]
                    anu = await fetchJson(`https://arugaz.herokuapp.com/api/jodohku?nama=${akuh}&pasangan=${kamuh}`)
			        hasil = `Nama : ${akuh}\nPasangan : ${kamuh}\n\nPositif : ${anu.positif}\nNegatif : ${anu.negatif}`
			    ...
			    case 'otagall3':
					...
					break
			    case 'kbbi':
					if (args.length < 1) return reply('hmmmmmmmm')
				...
				case 'bc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							sendMess(_.jid, `[ *GALIH BROADCAST* ]\n\n${body.slice(4)}`)
						}
						reply('Suksess broadcast')
					}
					break
				...
				case 'clone':
					...
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('di tag sayang')
					...
					try {
						...
						reply('ERROR, mungkin pp depresi')
					}
					break
				case 'setprefix':
				...
				//frendowner
				case `addadmin`:
                   if (!isOwner) return reply('Perintah ini hanya bisa di gunakan oleh Owner GALIH!', id)
                   for (let i = 0; i < mentionedJidList.length; i++) {
                   adminNumber.push(mentionedJidList[i])
                   fs.writeFileSync('./lib/database/admin.json', JSON.stringify(adminNumber))
                   reply('Success Menambahkan Admin GALIH!')
				   }
                   break
				case 'wait':
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						reply('Reply foto atau kirim foto dengan caption #wait')
					}
					break
				default:
